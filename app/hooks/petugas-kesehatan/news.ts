...
const baseUrl = "/api/petugas-kesehatan/dashboard/news";
const entity = "news";
const queryKey = "news";

export const useNewies = ({ queryString }: { queryString?: string }) => {
  ...
  const { data, isLoading: fetchLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return result.data.result as NewsDataModel[];
    },
  });

  const { mutateAsync: onCreate, isPending: onCreateLoading } = useMutation({
    mutationFn: async (payload: NewsPayloadCreateModel) =>
...
export const useNews = ({ id }: { id: string }) => {
  const queryClient = useQueryClient();
  ...
  const { mutateAsync: onUpdate, isPending: onUpdateLoading } = useMutation({
    mutationFn: async ({
      ...
      id: string;
      payload: NewsPayloadCreateModel;
