...
import {
  WaBlastPsikologiDataModel,
  WaBlastPsikologiPayloadCreateModel,
} from "@/app/models/program-wilayah/psikologi-blast";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import axios from "axios";

const baseUrl = "/api/program-wilayah/dashboard/psikologi-blast";
const entity = "psikologi-blast";
const queryKey = "psikologi-blasts";

export const usePsikologiBlasts = ({
  queryString,
}: {
  queryString?: string;
}) => {
  ...
  const { data, isLoading: fetchLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return result.data.result as WaBlastPsikologiDataModel[];
    },
  });

  const { mutateAsync: onCreate, isPending: onCreateLoading } = useMutation({
    mutationFn: async (payload: WaBlastPsikologiPayloadCreateModel) =>
...
export const usePsikologiBlast = ({ id }: { id: string }) => {
  const queryClient = useQueryClient();
