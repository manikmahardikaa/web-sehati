...
import { generateMonthlyReportBuffer } from "@/app/utils/monthly-distribution-buffer";
...
export async function POST() {
  try {
    ...
    const filename = `report-distribution-patient-${month}.pdf`;
    ...
    const payload = {
      name: `Laporan Distribusi Pasien - ${month}`,
      type_report: "MONTHLY_DISTRIBUTION_PATIENT",
      report_url: url,
    };
