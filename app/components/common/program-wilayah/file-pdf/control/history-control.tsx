...
export interface PatientControlReportProgramWilayah {
  name: string;
  no_whatsapp: string;
...
type Props = {
  ...
  printDate: string;
  data: PatientControlReportProgramWilayah[];
};

export const PatientControlProgramWilayahPDFDoc: React.FC<Props> = ({
  month,
  year,
  subDistrictName,
  staffName,
  ...
  const flattenedRows: {
    ...
    i: number;
    ch: PatientControlReportProgramWilayah["controlHistory"][number];
    ci: number;
  }[] = [];
  data.forEach((p, i) =>
