...
export default function FormBlastADS({
  patients,
  onSuccess,
}: FormBlastADSProps) {
  ...
  const { onCreate: createWaBlast, onCreateLoading: loading } = useAdsBlasts(
    {}
  );
    ...
    patients.forEach((patient) => {
      patient.medicationHistory.forEach((med: MedicationHistoryDataModel) => {
        ...
        const h3date = dayjs
          ...
          .subtract(3, "day")
          .format("YYYY-MM-DD");
        if (!map.has(h3date)) map.set(h3date, []);
        map.get(h3date)!.push(patient);
      });
    });
    return map;
  }, [patients]);
  ...
  const handleSubmit = async (fields: FormFields) => {
    ...
    try {
      ...
      const payload: WaBlastADSPayloadCreateModel = {
        phone_numbers: JSON.stringify(phone_numbers),
        message: formattedMessage,
        patient_id: selectedPatientObjs[0].id, // ambil 1 saja!
        petugas_lapangan_id: selectedPatientObjs[0].petugas_lapangan_id!,
  ...
  return (
    ...
    >
      <Row gutter={32}>
        ...
        <Col span={12}>
          <Title level={5}>
            Pilih Pasien Berdasarkan Tanggal H-3 Pengambilan Obat
          </Title>
          {/* Date Picker */}
          <DatePicker
